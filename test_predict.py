...
from Data_Augmentation2D import normalize, get_roirect2, get_image_list, get_roirect_fix
...
if __name__ == '__main__':
    ...
    data_csv = '../datalist_25.csv'
    note = 'results_25_aug_1128'
    modelpath = './log_25_patients_aug_1126/UNet.hdf5'
    lognoteDir = './' + note
    ...
    for index in testlocs:
        ...
        if augment:
            ...
            minxlist1, maxxlist1, minylist1, maxylist1 = get_roirect(label_rotate, cols, rows, para_pad, para_hwratio, flag_cut = False)
            minx = minxlist1[0]
            maxx = maxxlist1[0] + 1
            miny = minylist1[0]
            maxy = maxylist1[0] + 1
            ...
            area_pre = np.sum(pred_binary)
            plaque_areas_pred.append(area_pre)
            area_label = np.sum(label_roi/255)
            plaque_areas_label.append(area_label)
            error_plaque_areas.append(area_label-area_pre)
            miny_save.append(miny)
            minx_save.append(minx)
            maxy_save.append(maxy)
            maxx_save.append(maxx)
    ...
    plt.figure()
    plt.scatter(plaque_areas_label, error_plaque_areas, c='r', marker='o')
