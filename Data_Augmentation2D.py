...
def get_roirect_fix(label, para_pad, wim, him, para_hwratio = 0.7, cut_hwth = 0.33, flag_cut = False):
...
def get_roirect2(label, para_pad, wim, him, para_hwratio = 0.5):
    ...
    # wroi = int(maxx-minx+1)
    # hroi = int(maxy-miny+1)
    #
    # if hroi>wroi:
    #     minx = int(centx-hroi/2)
    #     maxx = int(centx+hroi/2)
    # else:
    #     miny = int(centy-wroi/2)
    #     maxy = int(centy+wroi/2)
...
def get_data(datalocs, Datafilelist, norm_size, augment=True, flag_freesize = False):
    ...
    for index in datalocs:
        ...
        minxlist, maxxlist, minylist, maxylist = get_roirect(label, para_pad, cols, rows, para_hwratio)#, flag_cut = False
        ...
        for ind_sub in range(len(minxlist)):
            ...
            # for miny_roi in range(len(roi_list)):
            #     for minx_roi in range(len(roi_list)):
            #         for maxy_roi in range(len(roi_list)):
            #             for maxx_roi in range(len(roi_list)):
            #                 image_roi = image[miny-roi_list[miny_roi]:maxy+roi_list[maxy_roi], minx-roi_list[minx_roi]:maxx+roi_list[maxx_roi]]
            #                 label_roi = label[miny-roi_list[miny_roi]:maxy+roi_list[maxy_roi], minx-roi_list[minx_roi]:maxx+roi_list[maxx_roi]]
        ...
        if augment:
            for ind_flid in range(len(para_flip)):
                ...
                minxlist, maxxlist, minylist, maxylist = get_roirect(label_flip, para_pad, cols, rows, para_hwratio)#flag_cut = False
                for ind_sub in range(len(minxlist)):
                    minx = minxlist[ind_sub]
                    maxx = maxxlist[ind_sub] + 1
                    miny = minylist[ind_sub]
                ...
                minxlist, maxxlist, minylist, maxylist = get_roirect(label_rotate, para_pad, cols, rows, para_hwratio)#flag_cut = False
                for ind_sub in range(len(minxlist)):
                    minx = minxlist[ind_sub]
                    maxx = maxxlist[ind_sub] + 1
                    miny = minylist[ind_sub]
...
def load_data(data_csv, trainpart, shape):
  ...
  # trainlocs = [i for i in range(int(num_files * 0.67))]
  # testlocs = [i for i in range(int(num_files* 0.67), num_files)]
  trainlocs = [i for i in range(946)]
  testlocs = [i for i in range(946, num_files)]
...
if __name__=='__main__':
  ...
  trainimagelist, trainlabellist, testimagelist, testlabellist = load_data('../Data_prep/datalist_25_patients.csv', 1, shape=(96,144))
  ...
  savepath = './npydata_25patients_aug_1126'
